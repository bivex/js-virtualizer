...
const implOpcode = {
    LOAD_BYTE: function () {
        ...
        for (let i = 0; i < numEntries; i++) {
            ...
            if (indexValue === caseValue) { this.write(indexReg, stateId); return; }
        }
        ...
        for (let i = 0; i < numEntries; i++) {
            ...
            const stateId = this.readDWORD();
            if (indexValue === scrambledKey) { this.write(shiftReg, stateId); return; }
        }
        const defaultState = this.readDWORD();
        ...
        const encrypted = typeof Uint8Array !== "undefined" && src instanceof Uint8Array ? Buffer.from(src) : src;
        ...
        if (this.executionMode === "async") {
            return (async () => { await fork.runAsync(); const rr = fork.registers; for (let i = 0; i < this.registers.length && i < rr.length; i++) this.registers[i] = rr[i]; })();
        }
        fork.run();
        const rr = fork.registers;
        for (let i = 0; i < this.registers.length && i < rr.length; i++) this.registers[i] = rr[i];
    }, DYN_PATCH: function () {
        const srcReg = this.readByte();
        ...
        const patch = src instanceof Uint8Array ? src : new Uint8Array(src);
        this.code = new Uint8Array(this.code);
        for (let i = 0; i < length && patchOffset + i < this.code.length; i++) this.code[patchOffset + i] = patch[i];
        ...
        for (let r = 0; r < numRegions; r++) {
            ...
            for (let i = perm.length - 1; i > 0; i--) { state = Math.imul((state ^ (state >>> 15)) >>> 0, 0x2c1b3c6d) >>> 0; state = (state + 0x9e3779b9 + i) >>> 0; const j = state % (i + 1); [perm[i], perm[j]] = [perm[j], perm[i]]; }
