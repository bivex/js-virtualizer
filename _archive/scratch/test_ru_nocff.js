...
async function run() {
    for (const [label, cff] of [['cff-on', true], ['cff-off', false]]) {
        const tempDir = fs.mkdtempSync(path.join(__dirname, '..', 'output', 'ru-'));
        const outPath = path.join(tempDir, 'test.out.js');
        await transpile(source, {
            codeInterleaving: true, controlFlowFlattening: cff,
            ...
            passes: ["RemoveUnused"], fileName: `ru-${label}`,
