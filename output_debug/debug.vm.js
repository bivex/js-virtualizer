...
function createBytecodeIntegrityDigest(payload, salt, key, format) {
    ...
    let a = (608135816 ^ input.length) >>> 0;
    let b = (2242054355 ^ normalizedSalt.length) >>> 0;
    let c = (320440878 ^ normalizedFormat.length) >>> 0;
    let d = (57701188 ^ normalizedKey.length) >>> 0;
    for (let i = 0; i < input.length; i++) {
        ...
        a = Math.imul((a ^ code + keyCode + i) >>> 0, 73244475) >>> 0;
        b = rotateLeft(b + code + saltCode + i >>> 0, 5);
        b = Math.imul((b ^ a) >>> 0, 668265261) >>> 0;
        c = rotateLeft((c ^ b + code + keyCode) >>> 0, 11);
        c = Math.imul(c >>> 0, 374761393) >>> 0;
        d = rotateLeft(d + (code ^ c) + saltCode >>> 0, 17);
        d = Math.imul((d ^ a ^ keyCode) >>> 0, 2654435761) >>> 0;
    }
    a ^= b >>> 1;
    b ^= c >>> 3;
    c ^= d >>> 5;
...
function createSeedFromString(input, initial = 2654435769) {
    const normalizedInput = String(input ?? '');
    let seed = initial >>> 0;
    for (let i = 0; i < normalizedInput.length; i++) {
        seed = Math.imul((seed ^ normalizedInput.charCodeAt(i) ^ i) >>> 0, 73244475) >>> 0;
...
function createSeededPermutation(length, seed) {
    ...
    for (let index = permutation.length - 1; index > 0; index--) {
        state = Math.imul((state ^ state >>> 15) >>> 0, 739982445) >>> 0;
        state = state + 2654435769 + index >>> 0;
...
function encodeUtf8(value) {
    ...
    for (let i = 0; i < normalizedValue.length; i++) {
        bytes[i] = normalizedValue.charCodeAt(i) & 255;
    }
    return bytes;
}
...
function createBytecodeCipherBytes(input, key, salt) {
    ...
    let stateA = createSeedFromString(`${ normalizedKey }:${ normalizedSalt }`, 608135816);
    let stateB = createSeedFromString(`${ normalizedSalt }:${ normalizedKey }`, 2242054355);
    for (let i = 0; i < data.length; i++) {
        const keyCode = normalizedKey.charCodeAt(i % normalizedKey.length);
        const saltCode = normalizedSalt.length > 0 ? normalizedSalt.charCodeAt(i % normalizedSalt.length) : 0;
        stateA = Math.imul((stateA ^ keyCode ^ i) >>> 0, 668265261) >>> 0;
        stateA = rotateLeft(stateA, 5);
        stateB = Math.imul(stateB + saltCode + i + (stateA & 255) >>> 0, 374761393) >>> 0;
        stateB = rotateLeft(stateB, 9);
        output[i] = data[i] ^ (stateA ^ stateB ^ keyCode ^ saltCode ^ i * 17) & 255;
    }
    return output;
}
function deriveOpcodeStateSeed(key) {
    return createSeedFromString(`opcode:${ String(key ?? '') }`, 1831565813) || 1831565813;
}
function deriveJumpTargetSeed(key) {
    return createSeedFromString(`jump:${ String(key ?? '') }`, 521288629) || 521288629;
}
function deriveRuntimeDispatchSeed(key) {
    return createSeedFromString(`dispatch:${ String(key ?? '') }`, 1327217884) || 1327217884;
}
function deriveAntiDebugSeed(key) {
    return createSeedFromString(`anti-debug:${ String(key ?? '') }`, 2135587861) || 2135587861;
}
function deriveInstructionByteSeed(key) {
    return createSeedFromString(`instruction:${ String(key ?? '') }`, 777692429) || 777692429;
}
function createOpcodePositionMask(seed, position) {
    let state = (seed ^ Math.imul(position + 1 >>> 0, 2654435761)) >>> 0;
    state = rotateLeft(state, position % 23 + 5);
    state ^= state >>> 16;
    return state & 255;
}
function createJumpTargetByteMask(seed, position) {
    let state = (seed ^ Math.imul(position + 1 >>> 0, 668265261)) >>> 0;
...
function createInstructionByteMask(seed, instructionPosition, bytePosition) {
    let state = (seed ^ Math.imul(instructionPosition + 1 >>> 0, 1831565813) ^ Math.imul(bytePosition + 1 >>> 0, 73244475)) >>> 0;
...
function decodeStatefulOpcode(opcode, position, seed) {
    return (opcode ^ createOpcodePositionMask(seed >>> 0, position >>> 0)) & 255;
}
...
const DISPATCHER_VARIANTS = new Set([
    'permuted',
    'clustered',
    'striped'
]);
const RUNTIME_OPCODE_DERIVATION_MODES = new Set([
    'hybrid',
    'stateful',
    'position'
]);
...
function normalizeVMProfile(profile = {}) {
    ...
    normalized.dispatcherVariant = DISPATCHER_VARIANTS.has(profile.dispatcherVariant) ? profile.dispatcherVariant : DEFAULT_VM_PROFILE.dispatcherVariant;
    ...
    normalized.runtimeOpcodeDerivation = RUNTIME_OPCODE_DERIVATION_MODES.has(profile.runtimeOpcodeDerivation) ? profile.runtimeOpcodeDerivation : DEFAULT_VM_PROFILE.runtimeOpcodeDerivation;
    normalized.polyEndian = profile.polyEndian === 'LE' || profile.polyEndian === 'BE' ? profile.polyEndian : DEFAULT_VM_PROFILE.polyEndian || 'BE';
    return normalized;
}
...
function buildDispatchEntries(realEntries, realGroups, decoyEntries, profile, seed) {
    ...
    default: {
            ...
            const entryOrder = createSeededPermutation(entries.length, seed ^ 2135587861);
...
function deriveAliasIndex(profile, slotsLength, opcode, position, runtimeState, runtimeDispatchSeed) {
    ...
    case 'position':
        return createOpcodePositionMask(runtimeDispatchSeed || 1327217884, (position ^ opcode) >>> 0) % slotsLength;
    case 'stateful':
        return createOpcodePositionMask(runtimeState || runtimeDispatchSeed || 1327217884, opcode) % slotsLength;
    default: {
            const mixedState = (runtimeState || runtimeDispatchSeed || 1327217884) ^ rotateLeft(position + 1 >>> 0, opcode % 13 + 3);
...
function mixRuntimeOpcodeState(state, opcode, position, salt = 0) {
    let next = (state ^ Math.imul(opcode + 1 >>> 0, 73244475) ^ Math.imul(position + 1 >>> 0, 374761393) ^ salt) >>> 0;
    next = rotateLeft(next, opcode % 19 + 5);
    next = Math.imul((next ^ next >>> 16) >>> 0, 668265261) >>> 0;
    return next || 2654435769;
}
...
function createMemoryProtectionState(key, numProtected) {
    ...
    for (let i = 0; i < normalizedKey.length; i++) {
        seed = Math.imul((seed ^ normalizedKey.charCodeAt(i)) >>> 0, 73244475) >>> 0;
    }
    return {
        enabled: true,
        seed,
...
function createRegisterProtectionMask(seed, register) {
    return rotateLeft((seed ^ Math.imul(register + 1, 2654435761)) >>> 0, register % 29 + 3);
}
function createProtectedRegisterValue(state, register, value) {
    ...
    const laneSeed = (state.seed ^ Math.imul(state.laneEpoch, 2654435761)) >>> 0;
...
function restoreProtectedRegisterValue(state, register, value, options = {}) {
    ...
    const laneSeed = (state.seed ^ Math.imul(laneEpoch, 2654435761)) >>> 0;
...
const implOpcode = {
    ...
    VFUNC_CALL: function () {
        ...
        this.regstack.push([
            this.captureRegisterSnapshot(),
            returnDataStore,
            new Map(this.registerRefs)
        ]);
    ...
    VFUNC_SETUP_CALLBACK: function () {
        const cur = this.read(registers.INSTRUCTION_POINTER);
        const fnOffset = this.readDWORD(), dest = this.readByte(), returnDataStore = this.readByte(), isAsync = this.readBool(), hasDynamicThis = this.readBool(), thisRegister = this.readByte(), usesArguments = this.readBool(), argumentsRegister = this.readByte(), useRest = this.readBool(), argArrayMapper = this.readArrayRegisters(), argOrder = this.readArrayRegisters(), captureMappings = this.readArrayRegisters();
        const vm = this;
        ...
        const cb = isAsync ? async function (...args) {
...
class JSVM {
    ...
    refreshDispatchTable() {
        const realPermutation = createSeededPermutation(opNames.length, this.opcodeStateSeed || 2654435769);
        const realEntries = [];
        const realGroups = [];
        for (const opcode of realPermutation) {
            const aliasCount = this.vmProfile.aliasBaseCount + (this.vmProfile.aliasJitter > 0 ? createOpcodePositionMask(this.runtimeDispatchSeed || 1327217884, opcode) % (this.vmProfile.aliasJitter + 1) : 0);
            const group = [];
        ...
        for (let decoyIndex = 0; decoyIndex < decoyCount; decoyIndex++) {
            decoyEntries.push({
                kind: 'decoy',
                seed: (this.runtimeDispatchSeed ^ Math.imul(decoyIndex + 1, 2654435761)) >>> 0
            });
        }
        const entries = buildDispatchEntries(realEntries, realGroups, decoyEntries, this.vmProfile, this.runtimeDispatchSeed || 1327217884);
        ...
        for (let slot = 0; slot < entries.length; slot++) {
            ...
            this.dispatchHandlers[slot] = (() => {
                ...
                return () => {
                    this.runtimeOpcodeState = mixRuntimeOpcodeState(this.runtimeOpcodeState || this.runtimeDispatchSeed || 1327217884, seed & 255, slot, seed);
                };
            })();
    ...
    setBytecodeIntegrityKey(key) {
        ...
        this.runtimeOpcodeState = this.runtimeDispatchSeed || 1327217884;
        this.refreshDispatchTable();
        return this;
    }
    ...
    enableAntiDebug(key) {
        ...
        this.runtimeOpcodeState = mixRuntimeOpcodeState(this.runtimeOpcodeState || this.runtimeDispatchSeed || 1327217884, this.antiDebugState.seed & 255, 0, this.antiDebugState.seed);
        return this;
    }
    ...
    scrambleInstruction(startPos, endPos) {
        if (!this.code || endPos <= startPos)
            return;
        const mask = (this.selfModifySeed ^ Math.imul(startPos + 1 >>> 0, 374761393)) >>> 0 & 255;
        for (let i = startPos; i < endPos; i++) {
            this.code[i] ^= (mask ^ i * 7 & 255) & 255;
        }
    }
    restoreBytecodeRange(fromPos, toPos) {
        if (!this.codeBackup || toPos <= fromPos)
            return;
        for (let i = fromPos; i < toPos; i++) {
            const mask = (this.selfModifySeed ^ Math.imul(i + 1, 2654435761)) >>> 0 & 255;
    ...
    enableAntiDump(key) {
        ...
        let seed = 1779033703;
        for (let i = 0; i < normalizedKey.length; i++) {
            seed = Math.imul((seed ^ normalizedKey.charCodeAt(i)) >>> 0, 465361333) >>> 0;
        }
        this.antiDumpSeed = seed;
        return this;
    }
    scrubBytecodeRange(fromPos, toPos) {
        if (!this.code || toPos <= fromPos)
            return;
        for (let i = fromPos; i < toPos; i++) {
            const mask = (this.antiDumpSeed ^ Math.imul(i + 1, 2246822507)) >>> 0 & 255;
            this.code[i] = mask;
        }
    }
    runAntiDebugSweep(position) {
        ...
        if (state.suspicionScore > state.appliedSuspicion) {
            this.runtimeOpcodeState = mixRuntimeOpcodeState(this.runtimeOpcodeState || this.runtimeDispatchSeed || 1327217884, state.seed & 255, position, state.suspicionScore);
            state.appliedSuspicion = state.suspicionScore;
        }
    }
    advanceRuntimeOpcodeState(opcode, position) {
        this.runtimeOpcodeState = mixRuntimeOpcodeState(this.runtimeOpcodeState || this.runtimeDispatchSeed || 1327217884, opcode, position, this.antiDebugState ? this.antiDebugState.suspicionScore : 0);
    }
    ...
    rotateProtectedRegisters() {
        ...
        for (let register = registerNames.length; register < this.registers.length; register++) {
            ...
            const resolvedValue = restoreProtectedRegisterValue(this.memoryProtectionState, register, storedValue, { consume: storedValue && storedValue.__jsvmProtected === true });
    ...
    readStored(register) {
        ...
        const resolvedValue = restoreProtectedRegisterValue(this.memoryProtectionState, register, this.registers[register], { consume: true });
        this.registers[register] = createProtectedRegisterValue(this.memoryProtectionState, register, resolvedValue);
        return resolvedValue;
    }
    writeStored(register, value) {
        ...
        if (this.registers[register] && this.registers[register].__jsvmProtected === true) {
            restoreProtectedRegisterValue(this.memoryProtectionState, register, this.registers[register], { consume: true });
        }
        this.registers[register] = createProtectedRegisterValue(this.memoryProtectionState, register, value);
    }
    ...
    readByte() {
        ...
        const byte = this.code[position];
        this.registers[registers.INSTRUCTION_POINTER] = position + 1;
        if (byte !== undefined && this.perInstructionEncodingEnabled && this.instructionByteSeed && this.currentInstructionBase !== null && position > this.currentInstructionBase) {
    ...
    resolveOpcodeHandler(opcode, position = 0) {
        ...
        const aliasIndex = deriveAliasIndex(this.vmProfile, slots.length, opcode, position, this.runtimeOpcodeState, this.runtimeDispatchSeed);
    ...
    readJumpTargetDWORD() {
        ...
        if (!this.jumpTargetEncodingEnabled || !this.jumpTargetSeed) {
            return this.vmProfile && this.vmProfile.polyEndian === 'LE' ? bytes[0] | bytes[1] << 8 | bytes[2] << 16 | bytes[3] << 24 : bytes[0] << 24 | bytes[1] << 16 | bytes[2] << 8 | bytes[3];
        }
        const decoded = transformJumpTargetBytes(bytes, position, this.jumpTargetSeed);
        return this.vmProfile && this.vmProfile.polyEndian === 'LE' ? decoded[0] | decoded[1] << 8 | decoded[2] << 16 | decoded[3] << 24 : decoded[0] << 24 | decoded[1] << 16 | decoded[2] << 8 | decoded[3];
    }
    readFloat() {
        let binary = '';
        for (let i = 0; i < 8; ++i) {
    ...
    readString() {
        ...
        let str = '';
        for (let i = 0; i < length; i++) {
            str += String.fromCharCode(this.readByte() ^ length * 31 + i * 17 & 255);
        }
        return str;
    }
...
JSVM.registerBytecodeKey('JSVK_d890cf8fa7d6', 'Y7P8RDjX3/nZCnfvT+O7D/KMXTBguw3T');
JSVM.registerBytecodeKey('JSVK_50ae8f574b8b', 'iWs/Z0ywCTJrUwaoSQKOMyAlgawUUcuB');
