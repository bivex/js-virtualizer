...
- `stringEncryption` (bool, default `false`) - strengthens the bytecode string encoding with a key-derived per-build XOR layer on top of the existing position-based mask. Without this, string constants in the bytecode payload are recoverable by reversing the fixed `(length*31 + i*17)` mask. With it, each character byte is additionally XORed with `lcg(seed, i)` where seed is derived from `bytecodeIntegrityKey`, making static string extraction impossible without the key. The VM's `readString` method is replaced via an IIFE patch inserted after the class declaration.
- `registerEncryption` (bool, default `false`) - wraps the VM's `write` and `readStored` methods so that integer values stored in general-purpose registers (index > 2) are transparently XOR-encoded with a per-register mask derived from `bytecodeIntegrityKey`. Memory dumps of the register file yield only encrypted values; the plaintext is only visible during the single instruction that uses it. Non-integer values (strings, booleans, objects, null) bypass encryption to preserve JS semantics. The mask uses a 15-bit LCG hash keyed by register index, avoiding int32 overflow issues.
- `integrityCheck` (bool, default `false`) - injects a runtime tamper-detection guard that hashes the source of critical JSVM prototype methods (`write`, `readStored`, `read`, `readString`, `loadFromString`, `run`, `runAsync`) at class-definition time using FNV-1a keyed by `bytecodeIntegrityKey`. Before every `run()` / `runAsync()` call, the hashes are recomputed and compared against the expected values — any monkey-patch (e.g. `JSVM.prototype.write = spy`) changes the method source and causes an immediate `Error: JSVM integrity violation`. The expected hash table is XOR-tweaked with a per-build key-derived constant so a static signature database cannot be built without the key.
...
| Obfuscation | string constant encryption | ✅ | bytecode string constants double-encrypted with key-derived LCG layer on top of position mask; static string extraction requires the integrity key; opt-in via `stringEncryption: true` |
| Obfuscation | register file encryption | ✅ | integer register values XOR-encoded with per-register key-derived mask; memory dumps yield only encrypted values; opt-in via `registerEncryption: true` |
| Obfuscation | integrity self-check | ✅ | FNV-1a hashes of critical JSVM prototype methods verified before every run(); monkey-patches throw `JSVM integrity violation`; hash table keyed per-build; opt-in via `integrityCheck: true` |
...
| --- | --- | --- |
| Original JS | 0.09 ms | 1x |
| Light VM | 132 ms | ~1438x |
| Hardened VM (default) | 272 ms | ~2961x |
| Hardened VM + nested VM | 322 ms | ~3497x |
| Hardened VM, `memoryProtection: false` | 131 ms | ~1431x |
| Hardened VM, `memoryProtection: false` + nested VM | 146 ms | ~1590x |
| Hardened VM + advancedCFF | 313 ms | ~3401x |

`memoryProtection` uses array-backed storage with dirty-bit tracking — only registers that were written to since the last step are re-protected, instead of all 253 registers every step. Nested VM adds ~11% overhead when `memoryProtection` is disabled, and ~18% on the fully hardened profile.
